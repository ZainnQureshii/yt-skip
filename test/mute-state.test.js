...
class Media {
  ...
  fire() { this.listeners.slice().forEach((fn) => fn({ target: this })); }
  // Script writes .muted directly, so tests call this to emulate the browser event.
  setByUser(value) { this.muted = value; this.fire(); }
  setByVolume(value) { this.volume = value; this.fire(); }
}
...
media.fire(); // the browser event our write would have produced
